# Script to calibrate and plot calibration for each site using MAP prevalence at first, then to the data from Jen
...
# saveRDS(preir, 'M:/Kelly/postdoc_JoeC/pfs230/PrEIR_trialdata/PRmatch_draws.rds')
...
# Compare site file outputs, cali calibrated to lower, upper, and central, and target MAP pfpr values
lapply(admin1s_wdata, function(cc){
  # Get site-specific calibrated EIRs
  preir_temp <- preir %>% filter(site_name == cc)

  pfpr_data_temp <- pfpr_data[pfpr_data$site_name == cc,] %>%
    mutate(date = lubridate::my(paste(month, year, sep = '-')))
  target <- pfpr_data_temp$pf_positivity_rate / 100

  # Get parameters for specific site, using calibrated initial EIR
  params <- all_model_input[grep(cc, names(all_model_input), value = TRUE, ignore.case = TRUE)][[1]]$param_list
  parameters <- set_equilibrium(params, init_EIR = preir_temp$starting_EIR)

  message('running simulation for ', cc)
  raw <- run_simulation(parameters$timesteps, parameters = parameters)
  raw <- postie::drop_burnin(raw, burnin = 15 * 365)
  prev <- raw %>%
    postie::get_prevalence(
    ...
    mutate(date = lubridate::my(paste(month, year, sep = '-'))) %>%
    filter(year >= 2025) %>%
    summarize(lm_prevalence_5_9 = mean(lm_prevalence_5_9),
              lm_prevalence_9_18 = mean(lm_prevalence_9_18),
              .by = 'date')

  p <- ggplot() +
    geom_point(data = pfpr_data_temp,
               aes(x = date, y = pf_positivity_rate/100, color = age_group), size = 4) +
    geom_line(data = pfpr_data_temp,
               aes(x = date, y = pf_positivity_rate/100, color = age_group), linewidth = 0.5) +
    geom_line(data = prev, aes(x = date, y = lm_prevalence_5_9), color = '#4B244A') +
    geom_line(data = prev, aes(x = date, y = lm_prevalence_9_18), color = '#98C1D9') +
    scale_color_manual(values = c('5-8' = '#4B244A',
                                  '9-17' = '#98C1D9')) +
    labs(y ='Prevalence',
         x = "Time",
         color = NULL) +
  ...
  ggsave(paste0('M:/Kelly/postdoc_JoeC/pfs230/outputs/preir_validation/plot_', cc, '.png'), p)
}
)
